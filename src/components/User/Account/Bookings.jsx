...
function Bookings() {
  ...
  useEffect(() => {
    ...
    async function invoke(){
      ...
      const found = data.bookingData; //destructure _id from res and checking
      if (found) {
              setLoading(false);
              setBookings(found);   
            }
...
{  !loading?    (bookings?.length > 0 &&
  ...
  booking?.status === "Booked" && new Date(booking.checkIn) > new Date() ? (
    <button onClick={() => cancelBook(booking._id)} className="font-medium border flex gap-2 rounded-xl text-white hover:bg-red-500 py-2 shadow bg-red-400 px-2">
